...
void print_mynumber(char const * s, mynumber tmp) {
    printf("%70s = %50.40g : %0.8x %0.8x\n", s, tmp.x, tmp.i[HIGH_HALF], tmp.i[LOW_HALF]);
}

void print_double(char const * s, double x) {
    static mynumber tmp;
    tmp.x = x;
    print_mynumber(s, tmp);
}
