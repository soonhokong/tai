...
/*---------------------------- 0.25<|x|< 0.855469---------------------- */
    ...
    double u   = x > 0 ? big + x : big - x;
    y   = (x > 0) ? x - (u - big) : x + (u - big);
    double xx  = y*y;
    double s   = y + y * xx * (sn3 + xx * sn5);
    double c   = xx * (cs2 + xx * (cs4 + xx * cs6));
    int    k   = low_half(u) << 2;

    double sn  = (x > 0) ? __sincostab.x[k]   : - __sincostab.x[k];
    double ssn = (x > 0) ? __sincostab.x[k+1] : - __sincostab.x[k+1];
    double cs  = __sincostab.x[k+2];
    double ccs = __sincostab.x[k+3];

    double cor = (ssn+s*ccs-sn*c)+cs*s;
    double res = sn+cor;
    cor = (sn-res)+cor;
    return (res == res + 1.096 * cor) ? res : -9999;
}
